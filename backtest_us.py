...
def detect(df):
    ...
    for li in range(half):
        ...
        if li >= 20:
            ...
            if pre_low > 0 and (lh / pre_low - 1) > 0.40:
                continue
...
if __name__ == "__main__":
    if not os.path.exists(US_DIR):
        send("raw_data/us/ 없음! collect_ohlcv.py 먼저 실행하세요.")
        exit(1)
    ...
    meta_map = {str(r["ticker"]): r for _, r in meta_df.iterrows()}
    ...
    send(f"🇺🇸 미장 백테스트 시작 (EODHD)\n기간: {LOOKBACK_DAYS//365}년\n조건: 트렌드+컵핸들")

    files       = [f for f in os.listdir(US_DIR) if f.endswith(".csv")]
    all_signals = []
    ...
    for i, fname in enumerate(files):
        ...
        # 페니스탁 필터
        if float(df["Close"].iloc[-1]) < 5: continue
        ...
        for j, sig_ts in enumerate(idx):
            ...
            # RS 계산 (조건 아님, 피처용)
            rs = calc_rs(sl, spy_df, j) if spy_df is not None else 0.0
    ...
    df_out.to_csv("backtest_us_raw.csv", index=False, encoding="utf-8-sig")
    send(f"🇺🇸 미장 백테스트 완료\n{len(all_signals)}건\n저장: backtest_us_raw.csv")
